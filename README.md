...
So I returned to the Rust version, adding the second charged move to the movesets and creating the concept of team, trying to compute every possible team match.<br/>
...
* This simulator handles changes, so only the opener is a fixed slot in a team. What does it means? That given 3 Pokémon A, B and C, the simulator only takes in account a single combination between ABC and ACB, another single combination between BAC and BCA, and another single combination between CAB and CBA.
...
* In the case of both Pokémon launching a charged move in the same turn, the simulator checks for attack value of both Pokémon and gives piority to he highest one, like the game. In case of equal values, it gives priority to team2 instead of randomly choosing like the game (worst case scenario FTW).
...
```
12536 => {
 DIALGA DRAGON_BREATH_FAST IRON_HEAD THUNDER
 DRAGONITE DRAGON_BREATH_FAST DRAGON_CLAW DRACO_METEOR
 MEWTWO PSYCHO_CUT_FAST ICE_BEAM SHADOW_BALL
}
12538 => {
 TOGEKISS CHARM_FAST ANCIENT_POWER AERIAL_ACE
 GIRATINA SHADOW_CLAW_FAST DRAGON_CLAW SHADOW_SNEAK
 DIALGA DRAGON_BREATH_FAST IRON_HEAD THUNDER
}
12553 => {
 MEWTWO PSYCHO_CUT_FAST ICE_BEAM SHADOW_BALL
 TYRANITAR SMACK_DOWN_FAST CRUNCH STONE_EDGE
 DIALGA DRAGON_BREATH_FAST IRON_HEAD THUNDER
}
12588 => {
 DIALGA DRAGON_BREATH_FAST IRON_HEAD THUNDER
 GIRATINA_ORIGIN SHADOW_CLAW_FAST OMINOUS_WIND SHADOW_BALL
 SCIZOR BULLET_PUNCH_FAST X_SCISSOR NIGHT_SLASH
}
12625 => {
 GIRATINA_ORIGIN SHADOW_CLAW_FAST OMINOUS_WIND SHADOW_BALL
 GARCHOMP MUD_SHOT_FAST OUTRAGE EARTHQUAKE
 MEWTWO PSYCHO_CUT_FAST FLAMETHROWER PSYSTRIKE
}
12634 => {
 GIRATINA_ORIGIN SHADOW_CLAW_FAST OMINOUS_WIND SHADOW_BALL
 MELMETAL THUNDER_SHOCK_FAST ROCK_SLIDE SUPER_POWER
 GARCHOMP MUD_SHOT_FAST OUTRAGE EARTHQUAKE
}
12663 => {
 GARCHOMP MUD_SHOT_FAST OUTRAGE EARTHQUAKE
