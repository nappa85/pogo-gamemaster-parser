...
pub async fn exec(filename: &str) -> Result<(), ()> {
    ...
    let filter = import(filename, &movesets).await?;
    let teams2 = filter.clone().par_bridge()
        .map_with(&filter, |ms, m1| ms.clone().par_bridge().map(move |m2| (m1, m2)))
        .flatten()
        .map_with(&filter, |ms, (m1, m2)| ms.clone().par_bridge().map(move |m3| (m1, m2, m3)))
        .flatten()
        .filter(|(t0, t1, t2)| {
            // unique pokemon
            t0.pokemon.unique_id != t1.pokemon.unique_id &&
                t0.pokemon.unique_id != t2.pokemon.unique_id &&
                t1.pokemon.unique_id != t2.pokemon.unique_id
        });
    ...
    let matches = teams1.map_with(teams2, |ts, t1| ts.clone().map(move |t2| (t1, t2)))
        ...
        .fold(HashMap::new, |mut dict, ((t0_0, t0_1, t0_2), (t1_0, t1_1, t1_2))| {
            let entry = match combat(&[t0_0.1, t0_1.1, t0_2.1], &[t1_0, t1_1, t1_2]) {
